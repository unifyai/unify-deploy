...
@custom_function()
async def run_matterport_sync_tick(
    full: bool = False,
    mock: bool = True,
) -> dict:
    """Run a single Matterport sync tick.

    Reads per-object watermarks from ``Matterport/Meta/SyncState``,
    dispatches each enabled object type's ``sync_*`` function,
    aggregates returned tables, and appends an audit row to
    ``Matterport/Meta/SyncRuns``.
    """
    import datetime as _dt
    import importlib

    from unify_deploy.assistant_deployments.integrations.packages.matterport.functions._config import (
        get_matterport_config,
    )
    from unify_deploy.assistant_deployments.integrations.packages.matterport.functions._sync_helpers import (
        get_state_watermark,
        load_sync_state,
        seconds_since,
    )
    ...
    if mock:
        from unify_deploy.assistant_deployments.integrations.packages.matterport.functions._sync_models import (
            sync_matterport_models,
        )
        from unify_deploy.assistant_deployments.integrations.packages.matterport.functions._sync_view_stats import (
            sync_matterport_view_stats,
        )

        m = await sync_matterport_models(mock=True)
        v = await sync_matterport_view_stats(mock=True)
        finished = _dt.datetime.now(tz=_dt.timezone.utc).isoformat()
        tables = {**m["tables"], **v["tables"]}
        tables["sync_state"] = [
            {"object_type": "models", "last_synced_at": finished, "last_status": "ok"},
            {
                "object_type": "view_stats",
    ...
    async def _dispatch(object_key: str) -> None:
        ...
        try:
            module = importlib.import_module(
                f"unify_deploy.assistant_deployments.integrations.packages."
...
@custom_function()
async def get_matterport_sync_state(mock: bool = True) -> dict:
    ...
    if mock:
        return {
            ...
            "latest_run": {
                ...
                "finished_at": "2026-04-30T09:00:18Z",
                "row_totals_json": "{'models': 14, 'view_stats_daily': 98}",
                "errors_json": "[]",
                "mode": "mock",
            },
        }
...
@custom_function()
async def probe_matterport_tier(force: bool = False, mock: bool = True) -> dict:
    """Discover which Matterport surfaces the active credentials cover.

    Caches results in ``Matterport/Meta/Capabilities`` for
    ``MATTERPORT_TIER_PROBE_TTL_SECONDS`` (default 24h).
    """
    import datetime as _dt
    ...
    if mock:
        return {
            ...
            "capabilities": {
                ...
                "view_stats": {"available": True, "status_code": 200},
                "mosaics": {"available": True, "status_code": 200},
                "tags": {"available": True, "status_code": 200},
            },
        }
    ...
    from unify_deploy.assistant_deployments.integrations.packages.matterport.functions._capabilities import (
        run_tier_probe,
    )
    from unify_deploy.assistant_deployments.integrations.packages.matterport.functions._sync_helpers import (
        seconds_since,
    ...
    try:
        dm.ingest(
            ...
            description="Most recent Matterport credential capability probe.",
