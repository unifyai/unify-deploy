...
def test_duplicate_defer_seconds_waits_past_steal_grace(monkeypatch) -> None:
    ...
    monkeypatch.delenv("UNITY_INGEST_LEASE_STEAL_GRACE_SECONDS", raising=False)
    monkeypatch.delenv("UNITY_DUPLICATE_DEFER_BUFFER_SECONDS", raising=False)
...
def test_duplicate_defer_seconds_short_fallback_when_expiry_unparseable(
    ...
    monkeypatch.setenv("UNITY_DUPLICATE_DEFER_JITTER_SECONDS", "0")
    monkeypatch.delenv("UNITY_DUPLICATE_DEFER_FALLBACK_SECONDS", raising=False)
...
def test_duplicate_defer_seconds_fallback_is_tunable(monkeypatch) -> None:
    monkeypatch.setenv("UNITY_DUPLICATE_DEFER_JITTER_SECONDS", "0")
    monkeypatch.setenv("UNITY_DUPLICATE_DEFER_FALLBACK_SECONDS", "45")

    assert entrypoint_ingest._duplicate_defer_seconds("") == 45


def test_lease_lifetime_cap_defaults(monkeypatch) -> None:
    monkeypatch.delenv("UNITY_INGEST_LEASE_MAX_LIFETIME_S", raising=False)
    monkeypatch.delenv("UNITY_INGEST_LEASE_MAX_EXTENSIONS", raising=False)
...
def test_lease_lifetime_cap_can_be_disabled(monkeypatch) -> None:
    monkeypatch.setenv("UNITY_INGEST_LEASE_MAX_LIFETIME_S", "0")
    monkeypatch.delenv("UNITY_INGEST_LEASE_MAX_EXTENSIONS", raising=False)
...
def test_lease_lifetime_cap_custom_values(monkeypatch) -> None:
    monkeypatch.setenv("UNITY_INGEST_LEASE_MAX_LIFETIME_S", "900")
    monkeypatch.setenv("UNITY_INGEST_LEASE_MAX_EXTENSIONS", "20")
...
def test_lease_lifetime_cap_ignores_garbage(monkeypatch) -> None:
    monkeypatch.setenv("UNITY_INGEST_LEASE_MAX_LIFETIME_S", "not-a-number")
    monkeypatch.setenv("UNITY_INGEST_LEASE_MAX_EXTENSIONS", "also-bad")
