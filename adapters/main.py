...
async def teams_notification_processor(request: Request):
    ...
    try:
        ...
        logger.info(
            f"sender resolve[message]: id={sender_id}, tenantId={sender_user.get('tenantId')}, "
            f"identityType={sender_user.get('userIdentityType')}, "
            f"email_present={bool(sender_user.get('email'))}, "
            f"upn_present={bool(sender_user.get('userPrincipalName'))}, "
            f"fields={sorted(sender_user.keys())}",
        )

        # Fetch email from user profile if not in message
        if not sender_email and sender_id:
            user_data, user_status = await graph_get(
            ...
            logger.info(
                f"sender resolve[/users/{{id}}]: status={user_status}, "
                f"mail_present={bool((user_data or {}).get('mail'))}, "
                f"upn_present={bool((user_data or {}).get('userPrincipalName'))}, "
                f"resolved={bool(sender_email)}",
            )
        ...
        if not sender_email and sender_id and chat_id:
            members_data, members_status = await graph_get(
                f"https://graph.microsoft.com/v1.0/me/chats/{chat_id}/members",
            )
            members_list = (members_data or {}).get("value", [])
            matched = False
            for member in members_list:
                if member.get("userId") == sender_id:
                    ...
                    logger.info(
                        f"sender resolve[chat members]: matched member, "
                        f"email_present={bool(member.get('email'))}, "
                        f"upn_present={bool(member.get('userPrincipalName'))}, "
                        f"odata_type={member.get('@odata.type')}, "
                        f"roles={member.get('roles')}",
                    )
                    if sender_email:
                        break
            logger.info(
                f"sender resolve[/me/chats/{{id}}/members]: status={members_status}, "
                f"count={len(members_list)}, matched_sender_id={matched}, "
                f"resolved={bool(sender_email)}",
            )
        ...
        if not sender_email and sender_id:
            ...
            logger.info(
                f"sender resolve[fallback]: using synthetic {{sender_id}}@teams placeholder",
            )
