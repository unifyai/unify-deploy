...
def _task_due_queue_path() -> str:
    """Return the fully qualified live due queue path."""

    return _task_queue_path(SETTINGS.task_due_queue_name)
...
def _ensure_task_queue(queue_name: str) -> str:
    ...
    if queue_name == SETTINGS.task_due_queue_name:
        _task_due_queue_ensured = True
    return queue_path


def _ensure_task_due_queue() -> str:
    """Create the live scheduled-task queue on first use if it is missing."""

    return _ensure_task_queue(SETTINGS.task_due_queue_name)
...
def _create_or_adopt_task_run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    request_payload = {"project_name": "Unity", **payload}
...
def _validate_current_offline_activation(
    ...
    if request.source_type == "scheduled" and _normalize_datetime_string(
        ...
        (
            request.scheduled_for.astimezone(timezone.utc).isoformat()
...
async def _materialize_scheduled_task_activation(
    ...
    """Materialize one scheduled activation into its current delivery queue."""
    ...
    if (
        ...
        and (
            ...
            or previous_execution_mode != request.execution_mode
        )
    ):
