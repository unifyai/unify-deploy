...
function Install-NodeJS {
    ...
    # Install Bun (separate check - always runs even if Node.js was already installed)
    Write-Host ""
    ...
    # Install npx (separate check - always runs)
...
function Install-AgentService {
    ...
    # Configure Git safe directories (prevents "dubious ownership" error when
    # repo was created by SYSTEM but script is run manually by user)
    Write-Host "Configuring Git safe directories..."
    git config --global --add safe.directory C:/magnitude
    git config --global --add safe.directory C:/temp/unity
    git config --global --add safe.directory C:/agent-service
