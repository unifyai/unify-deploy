...
#
# This script runs on container start and:
# 1. Reads configuration from GCP metadata (or environment variables)
...
#
# Environment Variables (override metadata):
#   VNC_PASSWORD, HOSTNAME, GITHUB_TOKEN, ANTHROPIC_API_KEY,
#   UNIFY_KEY, UNIFY_BASE_URL, STAGING
# =============================================================================
...
# Convert metadata key to env var name (e.g., vnc-password -> VNC_PASSWORD)
get_config() {
    local key=$1
    local env_var=$(echo "$key" | tr '[:lower:]-' '[:upper:]_')
    local value=""
    
    # Check environment variable first (allows override)
    value="${!env_var}"
    
    # If not set, try GCP metadata
    if [[ -z "$value" ]]; then
...
VNC_PASSWORD=$(get_config "vnc-password")
CONFIG_HOSTNAME=$(get_config "hostname")
GITHUB_TOKEN=$(get_config "github-token")
ANTHROPIC_API_KEY=$(get_config "anthropic-api-key")
UNIFY_KEY=$(get_config "unify-key")
UNIFY_BASE_URL=$(get_config "unify-base-url")
...
# Generate VNC password file using Python (tigervncpasswd not always available)
...
def vnc_encrypt(password):
    ...
    # Password padded/truncated to 8 bytes (no bit reversal needed with this key)
...
elif [[ -d "/magnitude/package.json" ]]; then
...
echo "  - XFCE4 Desktop"
echo "  - TigerVNC Server (port 5900)"
...
if [[ "$CADDY_ENABLED" == "true" ]]; then
    ...
    echo "  Desktop: http://$IP_ADDR:6080/vnc.html"
