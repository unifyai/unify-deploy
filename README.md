...
- POST `/email/create`  
  ...
  Creates a Google Workspace user. Returns user resource.

- DELETE `/email/delete`  
  JSON body: `{ "primaryEmail": "user@domain" }`  
  Deletes the Google Workspace user.
...
- POST `/email/reply`  
  Receives a Pub/Sub push payload from Gmail watch topic. Fetches the latest message from history, then replies in-thread. Returns `{ "success": true, "replyId": "…" }`.
