...
- **Buckets:** `unity-assistant-archives` ✅ (per-assistant home `{id}.tar.gz`, ~1.2 GB) + legacy `unity-assistant-archives` (rollback). Code: `POOL_ASSISTANT_ARCHIVE_BUCKET = "unity-assistant-archives"`.
...
| 7 | **P1** | Referenced-but-unconfigured secrets | `PYPI_API_TOKEN` (unillm `pypi.yml`), `NEXT_SERVER_ACTIONS_ENCRYPTION_KEY` (console build-arg), `TWILIO_*`/`LIVEKIT_*`/`GCP_SA_KEY` (unity-deploy `hosted-tests.yml`) are not configured → empty/skip. |
