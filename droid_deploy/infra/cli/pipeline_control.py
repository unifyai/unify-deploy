...
Usage:
    ...
    python -m droid_deploy.infra.cli.pipeline_control list
    python -m droid_deploy.infra.cli.pipeline_control status --dispatch-id <id>
    python -m droid_deploy.infra.cli.pipeline_control cancel --dispatch-id <id>
    python -m droid_deploy.infra.cli.pipeline_control cancel --job-id <id>
    python -m droid_deploy.infra.cli.pipeline_control pause --dispatch-id <id>
    python -m droid_deploy.infra.cli.pipeline_control pause --job-id <id>
    python -m droid_deploy.infra.cli.pipeline_control resume --dispatch-id <id>
    python -m droid_deploy.infra.cli.pipeline_control resume --job-id <id>
    python -m droid_deploy.infra.cli.pipeline_control delete --dispatch-id <id> --confirm
    python -m droid_deploy.infra.cli.pipeline_control monitor --job-id <id>
    python -m droid_deploy.infra.cli.pipeline_control inspect --job-id <id>
    python -m droid_deploy.infra.cli.pipeline_control reconcile-dlq --env staging --ack
    python -m droid_deploy.infra.cli.pipeline_control retry --env production \\
...
def _apply_runtime_overrides(args: argparse.Namespace) -> None:
    ...
    if env:
        os.environ["DROID_GCP_PIPELINE_ENVIRONMENT"] = env
    if project:
        os.environ["DROID_PUBSUB_PROJECT_ID"] = project
...
def _load_job_snapshot(infra, job_id: str, *, include_events: bool = True):
    ...
    from droid_deploy.infra.gcp.pipeline_observability import is_fresh_lease
...
def _table_checkpoint_plan(
    *,
    ...
    if not manifest_key:
        return [], False
    try:
        from droid.common.pipeline import IngestPlan
...
async def cmd_pause(args: argparse.Namespace) -> None:
    ...
    """
    from droid_deploy.infra.gcp.message_parking import park_message
...
async def cmd_resume(args: argparse.Namespace) -> None:
    ...
    """
    from droid_deploy.infra.gcp.message_parking import (
        delete_parked,
        parked_payloads,
    )
    from droid_deploy.infra.gcp.work_queue import PubSubWorkQueue
