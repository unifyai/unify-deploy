"""Load the droid-deploy checkout ``.env`` without importing client packages.

Importing ``droid_deploy.assistant_deployments.clients.client_alpha`` (even to reach
...
package-qualified imports under ``droid_deploy.assistant_deployments.clients``.
"""
...
def droid_deploy_repo_root() -> Path:
    """Return the repository root (parent of the ``droid_deploy`` package)."""
