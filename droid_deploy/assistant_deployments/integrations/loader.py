...
from droid_deploy.assistant_deployments.scenarios.loader import load_scenario
from droid_deploy.assistant_deployments.scenarios.types import ScenarioSpec
...
class LoadedIntegration:
    ...
    ``droid.integration_status`` (runtime).  All list/dict values are
...
def _build_registry_row(manifest: IntegrationManifest) -> dict:
    ...
    * ``guidance_titles`` — titles of the package's guidance entries,
    ...
    .. note::
       ...
       (:mod:`droid.integration_status`) reads disk discovery directly,
