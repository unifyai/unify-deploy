...
Deployments opt into integrations by slug, and `droid-deploy` expands those
...
`droid-deploy` remains the Level 3 full-package path for custom runtime code.
...
The direction is intentionally simple: `droid-deploy` imports stable Droid
...
```
droid_deploy/assistant_deployments/integrations/
+-- __init__.py
...
Run integration tests from the `droid-deploy` repo with its own runner:
...
1. **FunctionManager registration smoke** -- invoke
   ``droid_deploy.assistant_deployments.scenarios.cli`` with ``--no-materialize
