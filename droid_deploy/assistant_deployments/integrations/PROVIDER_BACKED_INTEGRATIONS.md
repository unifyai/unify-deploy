...
`droid-deploy` still owns full integration packages under `integrations/packages/*` and `integrations/client_packages/*`. These packages are the right choice when Unify needs Python code, DataManager sync, custom guidance, client-specific logic, browser fallback, or deterministic tests.
...
- `droid-deploy` contributes a package only when Unify needs local code,
