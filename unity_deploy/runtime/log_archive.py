...
to gs://bucket/{namespace}/{job_name}/.
...
def upload_shutdown_logs(dry_run: bool = False) -> bool:
    if not JOB_NAME:
...
def main():
    upload_shutdown_logs(dry_run="--dry-run" in sys.argv)


class HostedShutdownLogBackend:
    def upload_shutdown_logs(self) -> bool:
        return upload_shutdown_logs()
