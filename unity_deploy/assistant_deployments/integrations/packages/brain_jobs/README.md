...
That command writes
`unity-deploy/unity_deploy/assistant_deployments/integrations/packages/brain_jobs/scenarios/brain_jobs_v0.yaml`
relative to the sibling unity-deploy checkout (configurable via
`--out`).  Commit + push the resulting YAML; the next unity-deploy
build picks it up.
