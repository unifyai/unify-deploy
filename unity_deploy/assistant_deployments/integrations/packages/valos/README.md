...
UK property valuation data layer for the ClientDelta Healthcare valuation
workflow. Replaces the original placeholder pass-through to
`api.valos.ai` with two upstream services that Unify holds accounts for:
...
> **Client-scoped, no Console card.** This package has no entry in the
> generic Console `INTEGRATION_PROVIDERS` registry. It is opted into a
> single client deployment via the `integrations=[...]` list on
> `BASE_SPEC.derive(...)` (ClientDelta — see
> `clients/clientdelta/deployments/v0/deployment.py`). Customers never see,
> configure, or hold the upstream credentials; both keys are
> Unify-owned and populated in the deployment env.
...
- **PropertyData**: ToS includes reseller / multi-tenant clauses. The
  package is single-tenant (ClientDelta) today, so this is moot — revisit
  before opting in any second client.
...
reset at the next call after midnight UTC. The package is single-tenant
today; cross-process aggregation is a follow-on if/when a second client
opts in.
