...
The map-rendering primitives live in ``render.py``.
"""
...
def _normalise_os_response(payload: dict, *, source: str) -> dict:
    """Flatten OS Names / OS Places response into a uniform shape."""
        ...
        flattened.append(
            {
                ...
                "easting": _safe_float(item.get("X_COORDINATE") or item.get("GEOMETRY_X")),
                "northing": _safe_float(item.get("Y_COORDINATE") or item.get("GEOMETRY_Y")),
                "uprn": item.get("UPRN"),
...
async def valos_postcode_demographics(
    ...
    supply / fee data the workflow ingests Carterwood reports separately
    via ``primitives.files.parse``.
    """
    import os
