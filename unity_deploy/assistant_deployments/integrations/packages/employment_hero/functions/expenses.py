...
async def list_expense_categories(mock: bool = True) -> dict:
    ...
    if mock:
        return {
            "categories": [
                {"id": "ec-1", "name": "Travel - Mileage",
                 "is_taxable": False, "default_currency": "GBP"},
                {"id": "ec-2", "name": "Materials", "is_taxable": True,
                 "default_currency": "GBP"},
                {"id": "ec-3", "name": "Subsistence", "is_taxable": False,
                 "default_currency": "GBP"},
            ],
        }
...
async def list_expense_claims(
    ...
    if mock:
        return {
            "claims": [
                {"id": "ex-mock-1", "employee_id": "emp-mock-2",
                 "category_id": "ec-1", "amount": 12.40, "currency": "GBP",
...
async def get_expense_claim(claim_id: str, mock: bool = True) -> dict:
    ...
    if mock:
        return {"id": str(claim_id), "employee_id": "emp-mock-2",
...
async def submit_expense_claim(
    ...
    if mock:
        return {"id": "ex-mock-new", "employee_id": employee_id,
                "amount": amount, "currency": currency, "status": "submitted",
                "_mocked": True}
    ...
    if err is not None:
        return err
    payload: dict = {
        "employee_id": employee_id, "category_id": category_id,
...
async def sync_expenses(mock: bool = False, since: str | None = None) -> dict:
    ...
    if mock:
        return {
            ...
            "tables": {
                "expense_categories": [{"id": "ec-1", "name": "Travel - Mileage",
                                         "is_taxable": False,
                                         "default_currency": "GBP"}],
                "expense_claims": [{"id": "ex-mock-1",
                                     "employee_id": "emp-mock-2",
                                     "category_id": "ec-1", "amount": 12.40,
                                     "currency": "GBP", "date": "2026-04-28",
                                     "status": "submitted", "updated_at": started}],
            },
            "metadata": {"integration": "employment_hero",
                         "object_type": "expenses", "started_at": started,
                         "mode": "mock"},
        }
