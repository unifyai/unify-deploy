...
async def list_reviews(
    ...
    if mock:
        return {"reviews": [
            {"id": "rev-1", "employee_id": "emp-mock-1",
             "reviewer_id": "emp-mock-3", "period": "2026Q1",
...
async def get_review(review_id: str, mock: bool = True) -> dict:
    if mock:
        return {"id": str(review_id), "employee_id": "emp-mock-1",
                "reviewer_id": "emp-mock-3", "period": "2026Q1",
                "rating": 4,
...
async def list_goals(
    ...
    if mock:
        return {"goals": [
            {"id": "goal-1", "employee_id": "emp-mock-1",
             "title": "Q2 occupancy ≥ 92% across Battersea Portfolio",
             "target_value": 92.0, "current_value": 88.5,
             "unit": "percent", "status": "in_progress",
             "due_date": "2026-06-30"},
        ]}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
...
async def get_goal(goal_id: str, mock: bool = True) -> dict:
    if mock:
        return {"id": str(goal_id), "title": "Q2 occupancy ≥ 92%",
                "target_value": 92.0, "current_value": 88.5,
                "unit": "percent", "status": "in_progress"}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
...
async def create_goal(
    ...
    if mock:
        return {"id": "goal-mock-new", "employee_id": employee_id,
                "title": title, "_mocked": True}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
...
async def list_one_on_ones(
    ...
    if mock:
        return {"one_on_ones": [
            {"id": "111-1", "employee_id": "emp-mock-1",
             "manager_id": "emp-mock-3", "scheduled_at": "2026-04-30T15:00:00Z",
...
async def get_one_on_one(one_on_one_id: str, mock: bool = True) -> dict:
    if mock:
        return {"id": str(one_on_one_id), "employee_id": "emp-mock-1",
...
async def create_one_on_one(
    ...
    if mock:
        return {"id": "111-mock-new", "employee_id": employee_id,
                "manager_id": manager_id, "scheduled_at": scheduled_at,
                "_mocked": True}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_post, org_path, _org_id_or_error,
    )
...
async def list_feedback(
    ...
    if mock:
        return {"feedback": [
            {"id": "fb-1", "subject_employee_id": "emp-mock-1",
             "author_id": "emp-mock-3", "type": "appreciation",
...
async def list_peer_feedback(
    ...
    if mock:
        return {"peer_feedback": [
            {"id": "pf-1", "subject_employee_id": employee_id,
             "author_id": "emp-mock-2", "type": "peer_review",
...
async def give_feedback(
    ...
    if mock:
        return {"id": "fb-mock-new",
                "subject_employee_id": subject_employee_id,
                "type": feedback_type, "_mocked": True}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_post, org_path, _org_id_or_error,
    )
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._config import (
...
async def sync_performance(
        ...
        return {
            ...
            "tables": {
                ...
                "goals": [{
                    ...
                    "target_value": 92.0, "current_value": 88.5,
                    "unit": "percent", "status": "in_progress",
                    "due_date": "2026-06-30", "updated_at": started,
                }],
                "one_on_ones": [{
                    "id": "111-1", "employee_id": "emp-mock-1",
                    "manager_id": "emp-mock-3",
                ...
                "feedback": [{
                    ...
                    "author_id": "emp-mock-3", "type": "appreciation",
                    "is_anonymous": False,
                    "body_length": 32, "body_hash": "eeee5555",
                    "created_at": "2026-04-12T10:00:00Z",
                    "updated_at": started,
                }],
            },
    ...
    revs_raw = await eh_paginate(
        org_path("/reviews"), params=rev_params,
    ...
    goals_raw = await eh_paginate(
        org_path("/goals"), params=goal_params,
        page_size=cfg["api_page_size"],
        max_pages=cfg["max_pages_per_sync"],
    )
    ...
    o_raw = await eh_paginate(
        org_path("/one_on_ones"), params=o_params,
        page_size=cfg["api_page_size"],
        max_pages=cfg["max_pages_per_sync"],
    )
    ...
    fb_raw = await eh_paginate(
        org_path("/feedback"), params=fb_params,
        page_size=cfg["api_page_size"],
        max_pages=cfg["max_pages_per_sync"],
    )
    feedback: list[dict] = []
    ...
    redacted_fields = [
        "reviews.self_assessment", "reviews.manager_feedback",
        "reviews.improvement_areas",
        "one_on_ones.discussion_notes", "one_on_ones.action_items",
        "feedback.body",
    ] if redact else []
    ...
    return {
        ...
        "tables": {
            ...
            "one_on_ones": one_on_ones, "feedback": feedback,
        },
        "metadata": {
            "integration": "employment_hero", "object_type": "performance",
            "organisation_id": org_id, "started_at": started,
            "finished_at": finished, "since": since,
            "redacted_fields": redacted_fields,
            "row_counts": {
                "reviews": len(reviews), "goals": len(goals),
                "one_on_ones": len(one_on_ones), "feedback": len(feedback),
            },
        },
    }
