...
async def list_courses(mock: bool = True) -> dict:
    if mock:
        return {"courses": [
            {"id": "course-1", "name": "Fair Housing & Equality (UK)",
             "duration_minutes": 45, "is_mandatory": True,
             "category": "compliance"},
        ]}
...
async def get_course(course_id: str, mock: bool = True) -> dict:
    if mock:
        return {"id": str(course_id), "name": "Fair Housing & Equality (UK)",
                "duration_minutes": 45, "is_mandatory": True}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
    )
...
async def list_course_assignments(
    ...
    if mock:
        return {"assignments": [
            {"id": "ca-1", "course_id": "course-1",
             "employee_id": "emp-mock-1",
             "assigned_at": "2026-04-01",
             "due_date": "2026-04-30", "status": "in_progress"},
        ]}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
    )
...
async def list_course_completions(
    ...
    if mock:
        return {"completions": [
            {"id": "cc-1", "course_id": "course-1",
             "employee_id": "emp-mock-3",
             "completed_at": "2026-04-08T11:30:00Z", "score": 92},
        ]}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
    )
...
async def sync_learning(mock: bool = False, since: str | None = None) -> dict:
        ...
        return {
            ...
            "tables": {
                "courses": [{"id": "course-1",
                              ...
                              "category": "compliance",
                              "updated_at": started}],
                "course_assignments": [{"id": "ca-1", "course_id": "course-1",
                                         "employee_id": "emp-mock-1",
                                         "assigned_at": "2026-04-01",
                                         "due_date": "2026-04-30",
                                         "status": "in_progress",
                                         "updated_at": started}],
                "course_completions": [{"id": "cc-1", "course_id": "course-1",
                                         "employee_id": "emp-mock-3",
    ...
    courses_raw = await eh_paginate(org_path("/courses"),
    ...
    assignments_raw = await eh_paginate(org_path("/course_assignments"),
                                          params=a_params,
                                          page_size=page_size,
                                          max_pages=max_pages)
    course_assignments = [{
        "id": a.get("id"), "course_id": a.get("course_id"),
    ...
    completions_raw = await eh_paginate(org_path("/course_completions"),
                                          params=c_params,
                                          page_size=page_size,
                                          max_pages=max_pages)
    course_completions = [{
        "id": c.get("id"), "course_id": c.get("course_id"),
    ...
    return {
        ...
        "metadata": {
            "integration": "employment_hero", "object_type": "learning",
