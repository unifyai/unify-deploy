...
async def list_jobs(
    ...
    if mock:
        return {"jobs": [
            {"id": "job-1", "title": "Maintenance Operative — Camden",
             "status": "open", "posted_at": "2026-04-01",
             "closes_at": "2026-05-15", "location_id": "loc-mock-2"},
        ]}
...
async def get_job(job_id: str, mock: bool = True) -> dict:
    if mock:
        return {"id": str(job_id),
                "title": "Maintenance Operative — Camden", "status": "open"}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
    )
    _, err = _org_id_or_error()
...
async def list_applicants(
    ...
    if mock:
        return {"applicants": [
            {"id": "app-1", "job_id": "job-1",
             "first_name": "Jordan", "last_name": "Field",
             "email": "jordan.field@example.test",
             "stage": "first_interview", "applied_at": "2026-04-10"},
        ]}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
    )
...
async def get_applicant(applicant_id: str, mock: bool = True) -> dict:
    if mock:
        return {"id": str(applicant_id), "job_id": "job-1",
                "first_name": "Jordan", "last_name": "Field",
...
async def list_offers(job_id: str | None = None, mock: bool = True) -> dict:
    if mock:
        return {"offers": [
            {"id": "off-1", "job_id": "job-1", "applicant_id": "app-1",
             "status": "extended", "extended_at": "2026-04-26"},
        ]}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_get, org_path, _org_id_or_error,
    )
...
async def sync_recruitment(mock: bool = False, since: str | None = None) -> dict:
        ...
        return {
            ...
            "tables": {
                "jobs": [{"id": "job-1",
                           ...
                           "updated_at": started}],
                "applicants": [{"id": "app-1", "job_id": "job-1",
                                 "stage": "first_interview",
                                 ...
                                 "updated_at": started}],
                "offers": [{"id": "off-1", "job_id": "job-1",
                             "applicant_id": "app-1",
                             "status": "extended",
                             "extended_at": "2026-04-26",
                             "updated_at": started}],
                "interview_stages": [{"id": "stage-1", "name": "Phone Screen",
                                       "order": 1, "updated_at": started}],
            },
            "metadata": {"integration": "employment_hero",
                         ...
                         "redacted_fields": [
                             ...
                             "applicants.email", "applicants.phone",
                         ]},
        }
    ...
    offers_raw = await eh_paginate(
        org_path("/offers"), params=o_params,
        page_size=page_size, max_pages=max_pages,
    )
    offers = [{
        "id": o.get("id"), "job_id": o.get("job_id"),
    ...
    return {
        ...
        "tables": {
            ...
            "offers": offers, "interview_stages": stages,
        },
        "metadata": {
            "integration": "employment_hero", "object_type": "recruitment",
            ...
            "row_counts": {
                "jobs": len(jobs), "applicants": len(applicants),
                "offers": len(offers), "interview_stages": len(stages),
            },
        },
    }
