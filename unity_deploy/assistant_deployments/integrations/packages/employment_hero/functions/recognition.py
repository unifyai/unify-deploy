...
async def list_cheers(
    ...
    if mock:
        return {"cheers": [
            {"id": "cheer-1", "from_employee_id": "emp-mock-3",
...
async def give_cheer(
    ...
    if mock:
        return {"id": "cheer-mock-new",
                "to_employee_id": to_employee_id, "value": value,
                "_mocked": True}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
...
async def list_hi_fives(
    ...
    if mock:
        return {"hi_fives": [
            {"id": "hi5-1", "from_employee_id": "emp-mock-2",
             "to_employee_id": "emp-mock-1",
...
async def give_hi_five(
    ...
    if mock:
        return {"id": "hi5-mock-new", "to_employee_id": to_employee_id,
                "_mocked": True}
    from unity_deploy.assistant_deployments.integrations.packages.employment_hero.functions._client import (
        eh_post, org_path, _org_id_or_error,
    )
...
async def list_recognition_awards(mock: bool = True) -> dict:
    if mock:
        return {"awards": [
            {"id": "aw-1", "name": "Property Manager of the Quarter",
...
async def sync_recognition(mock: bool = False, since: str | None = None) -> dict:
    ...
    cheers_raw = await eh_paginate(org_path("/cheers"), params=cparams,
    ...
    hi5_raw = await eh_paginate(org_path("/hi_fives"), params=hparams,
                                 page_size=page_size, max_pages=max_pages)
    hi_fives = [{
        "id": h.get("id"),
    ...
    aw_raw = await eh_paginate(org_path("/recognition_awards"),
                                page_size=page_size, max_pages=max_pages)
    awards = [{
        "id": a.get("id"),
        "name": a.get("name"),
    ...
    return {
        ...
        "metadata": {
            "integration": "employment_hero", "object_type": "recognition",
            "organisation_id": org_id, "started_at": started,
            "finished_at": finished, "since": since,
            "row_counts": {"cheers": len(cheers), "hi_fives": len(hi_fives),
                           "awards": len(awards)},
        },
    }
