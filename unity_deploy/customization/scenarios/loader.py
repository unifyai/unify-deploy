...
:class:`~unity_deploy.customization.scenarios.types.ScenarioActivation`
...
def materialise_scenario_activations(
    ...
    *,
    ...
    6. Builds an env-var overlay from ``object_intervals_override`` and
       ...
       entries the caller merges into the resolved customization.
    ...
    tuple[list[ScenarioSpec], list[Secret]]
        ...
        customization's secrets bundle.
    ...
    for activation in activations:
        ...
        if template_path is None:
            roots = [
                str(p) for p in (search_paths or [_BUILTIN_DIR, _CLIENT_DIR])
            ]
        ...
        for idx, task in enumerate(materialised.get("tasks", [])):
            ...
            if not isinstance(current_id, str) or _is_placeholder(current_id):
                task["id"] = f"{scenario_id}_tick_{idx}" if idx else f"{scenario_id}_tick"
        ...
        # into the resolved customization's secrets bundle.
        if activation.object_intervals_override:
            env_name = f"{slug.upper()}_SYNC_OBJECT_INTERVALS"
            value = ",".join(
                f"{k}:{int(v)}"
                for k, v in activation.object_intervals_override.items()
            )
            secrets.append(
                Secret(
                    name=env_name,
