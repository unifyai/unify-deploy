...
async def enroll_in_workflow(
    ...
    if mock:
        return {"status": "enrolled", "workflow_id": str(workflow_id),
...
async def unenroll_from_workflow(
    ...
    if mock:
        return {"status": "unenrolled", "workflow_id": str(workflow_id),
                "contact_email": contact_email}
...
async def sync_marketing_workflows(
    ...
    if mock:
        ...
        return {
            ...
            "metadata": {"object_type": "marketing_workflows", "mode": "mock",
                         "row_count": len(rows)},
        }
    ...
    while True:
        ...
        if "error" in body:
            return {"schema_version": schema_version, "error": body["error"],
                    "tables": {"marketing_workflows": rows},
                    "metadata": {"object_type": "marketing_workflows", "mode": "real",
