    ...
    if mock:
        return {
            ...
            "properties": {
                ...
                "price": "10000", "hs_sku": "PMM-MONTHLY",
...
async def search_products(query: str, limit: int = 10, mock: bool = True) -> dict:
    ...
    if mock:
        return {
            "results": [{
                ...
                "properties": {
                    "name": "Property Management - Monthly Retainer",
                    "hs_sku": "PMM-MONTHLY", "price": "10000",
                    "hs_object_id": "300",
                },
                "createdAt": "2025-09-01T09:00:00Z",
                "updatedAt": "2026-01-15T11:00:00Z",
    ...
    return await hubspot_search(
        "products", query=query, properties=default_props, limit=min(limit, 100),
    )
...
async def create_product(properties: dict, mock: bool = True) -> dict:
    ...
    if mock:
        base_props = {
            "name": "New Product", "price": "0", "hs_product_type": "service",
            "createdate": "2025-09-01T09:00:00Z",
