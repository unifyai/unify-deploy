...
async def create_custom_object_record(
    ...
    return await hubspot_post(f"/crm/v3/objects/{object_type}", {"properties": properties})
...
async def sync_custom_objects(
    ...
    from unity_deploy.customization.integrations.packages.hubspot.functions._normalize import (
        normalize_custom_object_schema, normalize_object,
    )
    ...
    if mock:
        ...
        return {
            ...
            "metadata": {"object_type": "custom_objects", "mode": "mock",
                         "schemas": len(schemas), "records": len(records)},
        }
    ...
    if "error" in schemas_body:
        return {"schema_version": schema_version, "error": schemas_body["error"],
                "tables": {"custom_object_schemas": [], "custom_object_records": []},
                "metadata": {"object_type": "custom_objects", "mode": "real",
                             "partial": True}}
    ...
    for s in raw_schemas:
        ...
        while True:
            ...
            if not after or (cfg["max_pages_per_sync"] and pages >= cfg["max_pages_per_sync"]):
                break
