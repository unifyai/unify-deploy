...
def normalize_owner(record: dict) -> dict:
    return {
        ...
        "team_id": record.get("teams", [{}])[0].get("id") if record.get("teams") else None,
        "archived": bool(record.get("archived", False)),
        "created_at": record.get("createdAt", ""),
        "updated_at": record.get("updatedAt", ""),
    }
...
def normalize_list(record: dict) -> dict:
    return {
        ...
        "size": record.get("additionalProperties", {}).get("hs_list_size") or record.get("size"),
        "created_at": record.get("createdAt", ""),
        "updated_at": record.get("updatedAt", ""),
...
def normalize_association(*, from_id: str, to_id: str, association_type: str,
                          from_object_type: str, to_object_type: str) -> dict:
    return {
        "from_id": from_id,
        "to_id": to_id,
...
def normalize_custom_object_schema(record: dict) -> dict:
    return {
        ...
        "secondary_display_properties": _stringify(record.get("secondaryDisplayProperties") or []),
