...
async def publish_hubdb_table(
    ...
    if not confirm:
        return {"error": "publish_hubdb_table requires confirm=True.",
                "table_id": str(table_id)}
...
async def sync_hubdb(
    ...
    if mock:
        ...
        return {
            ...
            "metadata": {"object_type": "hubdb", "mode": "mock",
                         "tables": len(tables_rows), "rows": len(rows)},
        }
    ...
    if "error" in tables_body:
        return {"schema_version": schema_version, "error": tables_body["error"],
                "tables": {"hubdb_tables": [], "hubdb_rows": []},
                "metadata": {"object_type": "hubdb", "mode": "real",
                             "tables": 0, "rows": 0, "partial": True}}
    ...
    for t in tables_body.get("results", []):
        ...
        while True:
            ...
            if after:
                params["after"] = after
            row_body = await hubspot_get(f"/cms/v3/hubdb/tables/{t['id']}/rows", params=params)
            if "error" in row_body:
                break
            for r in row_body.get("results", []):
                row = {
                    "table_id": str(t["id"]), "row_id": str(r.get("id", "")),
