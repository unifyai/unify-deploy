...
All three environments share the same Orchestra (staging backend at `internal.example.com`), GCP secrets, and Artifact Registry. The environment boundary is enforced by `DEPLOY_ENV` and its derived `ENV_SUFFIX`, which partition resource names at every layer.
...
5. Upload commit SHA to `gs://bucket/image_hash_preview.txt`
