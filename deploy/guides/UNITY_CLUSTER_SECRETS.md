...
| **External Secrets Operator** | Polls SM `latest` → `unity-secrets` per namespace (`refreshInterval: 1h`) |
...
Requires `helm`, `gcloud`, and cluster admin on the **unity** GKE cluster. Run once per cluster:
...
- Applies `ClusterSecretStore` `unity-gcp-secret-manager`
...
Per-environment `ExternalSecret` resources are applied automatically on Unity deploy via Cloud Build (`apply-unity-external-secrets` in `deploy/cloudbuild-staging.yaml` and `deploy/cloudbuild.yaml`). You can also apply manually:
...
2. Wait for ESO (`refreshInterval` 1h) or force an immediate SM → K8s sync:
   ...
   ```bash
   kubectl annotate externalsecret unity-secrets -n staging \
     force-sync=$(date +%s) --overwrite
   ```

3. **Restart Unity pods** — env vars are fixed at container start:
...
While ESO owns `unity-secrets`, `setup_k8s_config.py --update` refuses to overwrite it (see `reconcile.external-secrets.io/data-hash` on the Secret).
...
- Do not rotate only GitHub Actions / CI secrets for GKE runtime — those are separate from `unity-secrets`.
