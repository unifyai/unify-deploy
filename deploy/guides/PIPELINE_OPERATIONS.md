...
python -m unify_deploy.infra.cli.pipeline_control monitor \
  --job-id "$JOB_ID" --follow

python -m unify_deploy.infra.cli.pipeline_control cancel \
  --job-id "$JOB_ID"

python -m unify_deploy.infra.cli.pipeline_control inspect \
  --job-id "$JOB_ID"
```
