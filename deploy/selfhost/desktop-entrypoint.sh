...
export DROID_GATEWAY_URL="${DROID_GATEWAY_URL:-http://gateway:8001}"
export ORCHESTRA_URL="${ORCHESTRA_URL:-http://orchestra:8000/v0}"
export DROID_COMMS_URL="${DROID_COMMS_URL:-$DROID_GATEWAY_URL}"
...
update_agent_service_env() {
  ...
  python3 - "$env_file" "$UNIFY_KEY" "$ORCHESTRA_URL" "$DROID_COMMS_URL" "$DROID_GATEWAY_URL" "${UNIFY_MODEL:-}" <<'PY'
import sys
...
keys = {
    ...
    "DROID_GATEWAY_URL": gateway_url,
}
if unify_model:
    keys["UNIFY_MODEL"] = unify_model
    keys["DROID_AGENT_SERVICE_LLM_MODEL"] = unify_model
...
ensure_playwright_cache_for_unityuser() {
  ...
  local user_cache="/Droid/.cache/ms-playwright"
