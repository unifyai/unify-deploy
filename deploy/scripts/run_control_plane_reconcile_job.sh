...
usage() {
  cat <<'USAGE'
Usage:
  run_control_plane_reconcile_job.sh \
...
Creates a one-off Kubernetes Job that reconciles Unity deploy-time
control-plane state into Orchestra, waits for it, prints logs, and deletes it.
USAGE
}
...
print_job_logs() {
  local job_ref="$1"
  echo "Control-plane reconcile logs (${job_ref}):"
  kubectl logs "$job_ref" -n "$namespace" --all-containers=true || true
}

delete_job() {
  local job_ref="$1"
  echo "Deleting control-plane reconcile job ${job_ref}..."
  kubectl delete "$job_ref" -n "$namespace" --ignore-not-found=true --wait=false >/dev/null || true
}

echo "Creating control-plane reconcile job in namespace ${namespace}..."
...
while true; do
  ...
  if [[ "$complete_condition" == "True" ]]; then
    echo "Control-plane reconciliation completed."
  ...
  if [[ "$failed_condition" == "True" ]]; then
    echo "Control-plane reconciliation failed." >&2
    print_job_logs "$job_ref"
  ...
  if (( SECONDS >= deadline )); then
    echo "Timed out waiting ${timeout} for control-plane reconciliation." >&2
    print_job_logs "$job_ref"
    kubectl describe "$job_ref" -n "$namespace" || true
    delete_job "$job_ref"
    exit 1
