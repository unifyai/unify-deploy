...
## Where Droid Job creation actually lives (production)
...
publish vars, the `droid-status` / `droid-date` / `droid-image-hash` labels,
the `droid-idle` PriorityClass, plus a couple of bugs). It was deleted to
avoid drift confusion.

The canonical Droid Job manifest builder is:

* [`communication/infra/helpers.py:create_droid_job`](../../../communication/infra/helpers.py)
...
| [`scripts/dev/job_utils.py`](../dev/job_utils.py) | List / read / patch labels on existing Droid Jobs via `/infra/jobs`. |
...
```bash
# Create one staging Job (uses the canonical create_droid_job manifest).
curl -X POST "https://service.a.run.app/infra/job/create" \
  -H "Authorization: Bearer $ORCHESTRA_ADMIN_KEY" \
  -F "namespace=staging" \
  -F "image=us-central1-docker.pkg.dev/gcp-project-runtime/droid/droid-staging:<commit>"
```
...
| `setup_priority_classes.py` | Create cluster-wide PriorityClasses (`droid-idle`, etc.) | Once per cluster |
