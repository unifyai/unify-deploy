...
ENV="${1:-staging}"
PROJECT_ID="${UNIFY_PUBSUB_PROJECT_ID:-gcp-project-runtime}"
REGION="us-central1"
...
echo "  export UNIFY_GCP_PIPELINE_ENVIRONMENT=${ENV}"
echo "  export UNIFY_GCS_ARTIFACT_BUCKET=${BUCKET}"
echo "  export UNIFY_PUBSUB_PROJECT_ID=${PROJECT_ID}"
echo ""
...
echo "  python -m unify_deploy.infra.workers.entrypoint_parse"
echo ""
echo "  # Terminal 2: Ingest worker (needs UNIFY_KEY, USER_ID, ASSISTANT_ID)"
echo "  python -m unify_deploy.infra.workers.entrypoint_ingest --project Assistants"
echo ""
echo "  # Terminal 3: Submit a job"
echo "  python -m unify_deploy.infra.cli.pipeline_control submit \\"
echo "    --config path/to/pipeline_config.json --project MyProject"
echo ""
echo "  # Monitor / cancel / inspect"
echo "  python -m unify_deploy.infra.cli.pipeline_control monitor --job-id <id> --follow"
echo "  python -m unify_deploy.infra.cli.pipeline_control cancel --job-id <id>"
echo "  python -m unify_deploy.infra.cli.pipeline_control inspect --job-id <id>"
