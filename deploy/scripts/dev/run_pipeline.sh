...
# Source .env for UNIFY_GCS_ARTIFACT_BUCKET, UNIFY_PUBSUB_PROJECT_ID etc.
PRESET_WORKER_NS="${UNIFY_WORKER_NS:-}"
PRESET_PARSE_SUB="${UNIFY_PARSE_SUB:-}"
...
if [[ -n "$PIPELINE_ENV_ARG" ]]; then
  ...
  PIPELINE_ENV="${UNIFY_GCP_PIPELINE_ENVIRONMENT:-staging}"
fi
...
PUBSUB_PROJECT="${UNIFY_PUBSUB_PROJECT_ID:-gcp-project-runtime}"
if [[ -n "$PIPELINE_ENV_ARG" ]]; then
  ...
  WORKER_NS="${UNIFY_WORKER_NS:-$PIPELINE_ENV}"
  PARSE_SUB="${UNIFY_PARSE_SUB:-unity-parse-sub${ENV_SUFFIX}}"
  INGEST_SUB="${UNIFY_INGEST_SUB:-unity-ingest-sub${ENV_SUFFIX}}"
  DLQ_SUB="${UNIFY_DLQ_SUB:-unity-dead-letter-sub${ENV_SUFFIX}}"
  ARTIFACT_BUCKET="${UNIFY_GCS_ARTIFACT_BUCKET:-unity-pipeline-artifacts${ENV_SUFFIX}}"
fi
export UNIFY_GCP_PIPELINE_ENVIRONMENT="$PIPELINE_ENV"
export UNIFY_GCS_ARTIFACT_BUCKET="$ARTIFACT_BUCKET"
