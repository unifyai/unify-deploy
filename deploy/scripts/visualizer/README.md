...
- `.env` file in `unity/` with:
  ...
  - `ORCHESTRA_ADMIN_KEY` — admin key for AssistantJobs system project queries
...
uv run python -m scripts.visualizer --data-dir clientdelta_healthcare_data
```
...
uv run python scripts/download_session_data.py --org-id 2 --name clientdelta
