...
Installations are available in the `droid-desktop-assistant` repo.
...
- Build: `docker build -t droid-desktop -f desktop/Dockerfile .`
- Run: `docker run --rm -p 6080:6080 -p 5900:5900 -p 3000:3000 --env-file .env droid-desktop`
...
### Troubleshooting
- Make sure `DROID_COMMS_URL`, `ORCHESTRA_URL` and `UNIFY_KEY` are in your `.env` file when starting the Docker container.
- Also make sure `DROID_COMMS_URL`, `ORCHESTRA_URL` and `UNIFY_KEY` are in your `agent-service/.env` file on starting the server.
- When running with Actor, make sure `UNIFY_KEY` is present in your droid `.env` for the agent-service server auth to work.
