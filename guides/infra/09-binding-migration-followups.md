...
- Tighten Unity desktop-ready handling in `unity/unity/conversation_manager/domains/event_handlers.py` so missing `binding_id` fails closed when the current session already has a binding, and add regression tests for stale/missing-binding events.
- Update `unity/deploy/scripts/stress_test/cleanup_stale_jobs.py` to stop runtimes through `AssistantSession` desired state or use binding-scoped release data instead of assistant-wide release requests.
