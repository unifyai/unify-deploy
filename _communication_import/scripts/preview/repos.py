...
``communication`` under the user's local ``Unify`` directory.  The
``Repo`` dataclass is the unit of "a checkout we touch via git": it
introspects the local branch state and pushes / deletes feature
branches on the corresponding origin remote without disturbing the
...
"""Default sibling-repo root.  ``communication`` lives at
``<root>/communication`` and the others at peer paths."""
...
REPOS: tuple[Repo, ...] = (
    Repo("communication", "Comms App + Adapters control plane"),
    Repo("orchestra", "FastAPI backend + assistant DB"),
    Repo("unity", "Assistant runtime (in-container brain)"),
    Repo("unity-deploy", "Enterprise overlay applied to Unity base image"),
    Repo("console", "Next.js frontend"),
)

PASSTHROUGH_REPO_NAMES: tuple[str, ...] = ("communication", "unity", "console")
"""Sibling repos whose Cloud Build triggers must be hit for an end-to-end
preview to come alive.

``communication`` produces the tagged Comms App and Adapters revisions
(and ``BRANCH_TAG`` is what wires the Unity image override into every
spawned AssistantSession).  ``unity`` builds the per-branch Unity
overlay image and uploads its hash to GCS.  ``console`` deploys the
tagged frontend whose ``NEXT_PUBLIC_*`` peer URLs are pinned to the
matching tagged comms-app and adapters revisions.

``unity-deploy`` is intentionally omitted: the unity preview build
sub-triggers it via Cloud Build substitutions, so a pass-through
branch on origin would just duplicate work.  ``orchestra`` is omitted
because its preview build is opt-in and rarely needed — schema
